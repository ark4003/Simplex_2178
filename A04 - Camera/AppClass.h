...
private:
	...
	//sets camera's starting position
	vector3 cameraPosition = vector3(0.0f, 3.0f, 20.0f);
	//set the camera's starting view target
	vector3 cameraLook = vector3(0.0f, 3.0f, 19.5f);
	vector3 cameraUp = vector3(AXIS_Y);

	//how much to rotate the camera
	vector3 viewChange = vector3(0.0f, 0.0f, 0.0f);


	//directional vectors
	vector3 frontVector = cameraLook - cameraPosition;
	vector3 rightVector = frontVector *glm::angleAxis(1.5708f, vector3(0.0f, 1.0f, 0.0f));
	vector3 rearVector = (frontVector) * -1.0f;
	vector3 leftVector = (rightVector) * -1.0f;

	//keeps track of how much the camera has rotated.  I PROBABLY only care about y.
	vector3 rotationTracker = vector3(0.0f, 0.0f, 0.0f);

	//how much to change the camera position
	vector3 positionChange = vector3(0.0f, 0.0f, 0.0f);
	//how much to change the camera's target
	vector3 lookChange = vector3(0.0f, 0.0f, 0.0f);
