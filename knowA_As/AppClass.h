...
class Application
{
	...
	//OCTREE STUFF CURRENTLY UNIMPLEMENTED
	uint m_uOctantID = -1; //Index of Octant to display
	uint m_uObjects = 0; //Number of objects in the scene
	uint m_uOctantLevels = 0; //Number of levels in the octree

	//x coordinate value for grid placement
	int sPX = -10;
	//z coordinate value for grid placement
	int sPZ = -10;

	//Max and Min X and Z values on grid
	int sPXMax = 0;
	int sPXMin = 0;
	int sPZMax = 0;
	int sPZMin = 0;
	...
	//starting position for zombie
	vector3 zombPosition = vector3(0.0f, 0.0f, 0.0f);

	//starting position for Steve
	vector3 stevePosition = vector3(0.0f, 0.0f, 0.0f);

	//Position matrices for Steve and Zombie
	matrix4 steveMatrix = IDENTITY_M4;
	matrix4 zombMatrix = IDENTITY_M4;


	//a_sUniqueID of this cube
	String cubeLabel = "no label yet";

	//Label of the cube Steve is on at the start
	String steveCube = "Steve is Cubeless";
	//label of the cube Zombie is on at the start
	String zombCube = "Zombie is cubeless";

	//collection of the cubes on the grid

	//dimensions for the grid.  NOTE: make sure array length matches column and row length
	const int rowLength = 25;
	const int columnLength = 19;

	//const is NOT working here because I'm using pointers instead of an array.  Fix this when I have time.
	//collection of the cubes on the grid
	vector3* cubeGroup[19][25];
	//vector3* cubeGroup = new cubeGroup[columnLength][rowLength];

	//Necessary information for the cube that steve and zombie are on at the moment.  NOT the same as steve or zombie's current LOCATION
	vector3 currentZombCube = vector3(0.0f);
	vector3 currentSteveCube = vector3(0.0f);

	//What value the starting and ending cube in the path have in the cube collection.  I is relative Z position, J is relative X Position.  These need to be swapped when I have the time.
	//Collection can be traversed by changing the I and J values.
	int startI = -1;
	int startJ = -1;
	int endI = -1;
	int endJ = -1;
...
private:
	String m_sProgrammer = "Andrew Knowland - ark4003@rit.edu"; //programmer
...
#pragma endregion
	...
	//MOVE TO OWN CLASS WHEN WORKING
#pragma region AStar stuff
	/*
	USAGE: Generate a Path
	ARGUMENTS: 
	startPoint: the ID of the starting cube (Zombie's)
	endPoint: the ID of the ending cube (Steve's)
	OUTPUT: ---
	*/
	void GeneratePath(String a_StartPoint, String a_EndPoint);

	/*
	USAGE: Follow the generated path
	ARGUMENTS: 
	OUTPUT: ---
	*/
