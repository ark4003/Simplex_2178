...
void Application::ProcessKeyPressed(sf::Event a_event)
{
	...
	//used to make sure camera movement's smooth while discarding slight inaccuracies in mouse movement
	case sf::Keyboard::I:
		viewChange.x = viewChange.x + 0.01f;
		break;
	case sf::Keyboard::J:
		viewChange.y = viewChange.y - 0.01f;
		break;
	case sf::Keyboard::K:
		viewChange.x = viewChange.x - 0.01f;
		break;
...
void Application::CameraRotation(float a_fSpeed)
{
	...
	viewChange.x = fAngleX;
	viewChange.y = fAngleY;
	SetCursorPos(CenterX, CenterY);//Position the mouse in the center
}
//Keyboard
...
#pragma region Camera Position
	...
	{
		//DEPRECATED (kept around to easily change back to old method if the current one is considered hard-coding, although it shouldn't be.)
		//use keys to change camera position
		//positionChange.z = -0.5f;

		//DEPRECATED
		//ensure camera is still looking at same relative position (in front of it)
		//lookChange.z = -0.5f;

		//lets me know what RELATIVE direction I'm moving in at the moment
		frontMove = true;
		rearMove = false;
	}
